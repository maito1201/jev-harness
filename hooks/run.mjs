...
import { MODEL, PROMPT_QUESTIONS, WRITE_QUESTIONS, COMMAND_QUESTIONS, STOP_QUESTIONS, MAX_BLOCKS_PER_TURN, NEEDS_OUTCOME_CHECK, OFF_SCOPE_ASK, LIMITS } from "./questions.mjs";
...
async function onPrompt() {
  ...
  s.promptKind = kind;
  saveState();
  log({ event, kind, needs_outcome_check: round(needs), confidence: r.answers.prompt_kind?.confidence, usage: r.usage, latencyMs: r.latencyMs, prompt: prompt.slice(0, 500) });
...
async function onPreToolUse() {
  ...
  if (s.approved === false && s.request) {
    log({ event, denied: true, tool: input.tool_name, file: fp });
  ...
  s._preNew = { ...(s._preNew || {}), [rel(fp)]: isNew }; saveState(); // PostToolUse では書いた後なので、新規かはここで覚える
  if (!apiKey || !s.request) return;
  const r = await ask({ request: clip(s.request, LIMITS.request), agreed_outcome: s.agreed_outcome ? clip(s.agreed_outcome, LIMITS.outcome) : null, file: rel(fp), is_new: isNew }, WRITE_QUESTIONS, { apiKey, model: MODEL });
  const p = r.answers.off_scope?.noul ?? 0;
  log({ event, tool: input.tool_name, file: rel(fp), is_new: isNew, off_scope: round(p), usage: r.usage, latencyMs: r.latencyMs });
  if (p >= OFF_SCOPE_ASK) {
    out({ hookSpecificOutput: { hookEventName: "PreToolUse", permissionDecision: "ask", permissionDecisionReason: `jev-harness: ${rel(fp)} への書き込みは依頼の範囲外に見える（${p.toFixed(2)}）。必要なら理由を述べて進める` } });
  }
}
...
async function onPostToolUse() {
  ...
  if (/^(Write|Edit|MultiEdit|NotebookEdit|apply_patch)$/.test(tool)) {
    const fp = filePath(); if (!fp) return;
    const f = s.facts.files_written;
    if (!f.some((x) => x.file === rel(fp))) f.push({ file: rel(fp), is_new: s._preNew?.[rel(fp)] ?? false });
    saveState(); return;
  }
...
// ── helpers ──
function filePath() { return String(input.tool_input?.file_path || input.tool_input?.path || input.tool_input?.notebook_path || ""); }
function rel(fp) { const cwd = String(input.cwd || ""); return cwd && fp.startsWith(cwd + "/") ? fp.slice(cwd.length + 1) : fp; }
